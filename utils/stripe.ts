...
export async function createCheckoutLink(customer: string) {
  const checkout = await stripe.checkout.sessions.create({
    ...
    line_items: [
      {
        price: "price_1QWl3jRo3MqOikJs3yc7gHjX",
        quantity: 1,
      },
    ],
    mode: "subscription",
    ...
    return {
      ...
      message: isSubscribed
        ? `You have reached the maximum number of lesson plans (30) for this subscription period. Reset Date: ${resetDate}
            `
        : `You have reached the maximum number of lesson plan generations (10) for the free tier. You can generator more starting ${resetDate}`,
      remainingGenerations: 0,
    };
  }
